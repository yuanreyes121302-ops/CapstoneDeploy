...
<div class="tenant-container">
    ...
    <div class="rooms-section">
        ...
        @foreach ($property->rooms as $room)
            <div class="room-card mb-4 p-2 border rounded">
                <div class="room-header mb-2">
                    <div class="room-title h5">{{ $room->name }}</div>
                </div>

                @if ($room->images->isNotEmpty())
                    <div class="image-gallery mb-2">
                        <div class="room-images d-flex flex-wrap gap-2">
                ...
                @foreach($property->reviews as $review)
                    <div class="review-box">
                        ...
                        <p class="mb-1">
                            ...
                            @if(auth()->check() && auth()->user()->role === 'tenant' && auth()->id() === $review->tenant_id)
                                <!-- Delete Button -->
                                <form action="{{ route('reviews.destroy', $review->id) }}" method="POST" class="d-inline-block ms-2">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('bookings.store', $property->id) }}" method="POST" id="bookingForm">
                ...
                <div class="modal-body">
                    ...
                    <!-- Room Selection -->
                    @if ($property->rooms && $property->rooms->where('status', 'Available')->count() > 1)
                        <div class="mb-4">
                    ...
                    @elseif ($property->rooms && $property->rooms->where('status', 'Available')->count() === 1)
                        <div class="mb-4">
                            <h6 class="fw-bold">Room</h6>
                            @php
                                $availableRoom = $property->rooms->where('status', 'Available')->first();
                            @endphp
                            <input type="hidden" name="room_id" value="{{ $availableRoom->id }}">
                            <p class="form-control-plaintext">{{ $availableRoom->name }}</p>
                        </div>
                    @else
                        <div class="mb-4">
                            <p class="text-warning">No rooms available at this time. Please contact the landlord.</p>
